...
public class Point implements Comparable<Point> {
  ...
  public Comparator<Point> slopeOrder() {
    ...
    /* YOUR CODE HERE */
    return new SloperOder();
  }

  private class SloperOder implements Comparator<Point> {

    @Override
    public int compare(Point pointOne, Point pointTwo) {
      double pointOneValue = slopeTo(pointOne);
      double pointTwoValue = slopeTo(pointTwo);
      if (pointOneValue < pointTwoValue) {
        return -1;
      } else if (pointOneValue > pointTwoValue) {
        return 1;
      }
      return 0;
